...
module.exports = async function(platform, accessory, el, address, eoj) {
  ...
  this.updateValveHistory = async function () {
    const valve = this.state.targetHeatingCoolingState ?
	  (this.state[`currentTemperature0x${0xBB.toString(16)}`] - this.state[`currentTemperature0x${0xB3.toString(16)}`])/this.state[`currentTemperature0x${0xB3.toString(16)}`] + 50 : 0;
    this.historyService.addEntry(
      {time: Math.round(new Date().valueOf()/1000),
