...
module.exports = async function(log, api, accessory, el, address, eoj) {
  ...
  try {
    this.mqttclient = await MQTT.connectAsync('mqtt://localhost');
    this.log(`${this.displayName}: MQTT connection has been established successfully.`);
    ...
    this.log(`${this.displayName}: Failed to establish MQTT connection. ${e}`);
  }
  this.mqttpublish = async function(topic, message) {
	...
	let eoj = this.eoj;
	await this.mqttclient.publish(`${address}/${eoj[0]}:${eoj[1]}:${eoj[2]}/${topic}`, `${message}`)
    ...
    // } else {
    //   this.log(`${this.displayName}: MQTT connection has not been established yet.`)
    // }
    }
  }
