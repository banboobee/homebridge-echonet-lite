...
module.exports = async function(platform, accessory, el, address, eoj) {
  ...
  try {
    ...
    this.log(`${this.displayName}: Failed to retrieve propertyMaps. ${e}`)
    this.propertyMaps = {
      inf: [],
      set: [],
      get: []
  ...
  this.log('Air-conditioner %s was found under controller #%d', id, el.ix)
  this.log('Thermostat is used as air-conditioner service of %s.', id)
	...
	if (this.historyService) {
	  if (key == `currentTemperature0x${0xB3.toString(16)}`) {
	    ...
	    this.historyService.addEntry(
	      {time: Math.round(new Date().valueOf()/1000), setTemp: value})
	    await this.mqttpublish('targetTemperature', value)
	  } else if (key == 'targetHeatingCoolingState') {
	    await this.mqttpublish('mode', mqttmode[value])
	    await this.mqttpublish('targetTemperature', this.state[`currentTemperature0x${0xB3.toString(16)}`])
	    this.log.debug(`adding history of targetHeatingCoolingState.`, value * 25, id)
	    this.valveInterval = 1;
	    clearTimeout(this.valveTimer);
	    this.updateValveHistory();
	    // this.historyService.addEntry(
  ...
  const mqttmode = ['off', 'heat', 'cool', 'auto']
  this.mqttpublish = async function(topic, message) {
    if (platform.mqttclient) {
      try {
	let address = this.address;
	let eoj = this.eoj;
	await platform.mqttclient.publish(`homebridge-echonet-lite/${address}/${eoj[0]}:${eoj[1]}:${eoj[2]}/${topic}`, `${message}`)
  ...
  try {
    ...
    log(`Failed to get initial CurrentTemperature or CurrentRelativeHumidity. (${e})`)
  }
  
  this.updateThermoHistory = async function () {
    try {
	...
	await this.mqttpublish('temperature', this.state[`currentTemperature0x${0xBB.toString(16)}`])
    ...
    setTimeout(() => {
      this.updateThermoHistory();
    }, 3 * 60 * 1000);
  }
  this.updateThermoHistory();

  this.updateValveHistory = async function () {
    let valve = this.state.targetHeatingCoolingState ?
	(this.state[`currentTemperature0x${0xBB.toString(16)}`] - this.state[`currentTemperature0x${0xB3.toString(16)}`])/this.state[`currentTemperature0x${0xB3.toString(16)}`]*100*2 + 50 : 0;
    valve = valve < 0 ? 0 : (valve > 100 ? 100 : valve);
    this.historyService.addEntry(
      {time: Math.round(new Date().valueOf()/1000),
       setTemp: this.state[`currentTemperature0x${0xB3.toString(16)}`],
       valvePosition: valve});
    // await this.mqttpublish('targetTemperature', this.state[`currentTemperature0x${0xB3.toString(16)}`] || 30)
    // await this.mqttpublish('mode', mqttmode[this.state.targetHeatingCoolingState])
	  ...
	  if (epc == 0x80) {
		...
		(mode == 3 ? hap.Characteristic.TargetHeatingCoolingState.HEAT :
		 hap.Characteristic.TargetHeatingCoolingState.COOL)
	      ...
	      if (state === hap.Characteristic.TargetHeatingCoolingState.AUTO) {
		if (this.state[`currentTemperature0x${0xB3.toString(16)}`] > this.state[`currentTemperature0x${0xBB.toString(16)}`]) {
	    ...
	    if (this.state.targetHeatingCoolingState === hap.Characteristic.TargetHeatingCoolingState.AUTO) {
	      if (this.state[`currentTemperature0x${0xB3.toString(16)}`] > this.state[`currentTemperature0x${0xBB.toString(16)}`]) {
		state = hap.Characteristic.CurrentHeatingCoolingState.HEAT;
	      } else {
		state = hap.Characteristic.CurrentHeatingCoolingState.COOL
...
//	  }
  ...
  .on('get', async function(callback) {
	...
	const {mode} = (await EL(el.getPropertyValue, this.address, this.eoj, 0xB0)).message.data
        if (mode === 1)
	  x = (this.state[`currentTemperature0x${0xB3.toString(16)}`] > this.state[`currentTemperature0x${0xBB.toString(16)}`]) ?
	  hap.Characteristic.CurrentHeatingCoolingState.HEAT :
	  hap.Characteristic.CurrentHeatingCoolingState.COOL
        else if (mode === 3)
	  x = hap.Characteristic.TargetHeatingCoolingState.HEAT
        else
	  x = hap.Characteristic.TargetHeatingCoolingState.COOL
      }
      if (this.state.currentHeatingCoolingState != undefined) {
	c.updateValue(x)
      } else {
