...
class ELPlatform {
  ...
  async _init() {
	...
	if (existingAccessory) {
	  ...
	  try {
	    ...
	    // duplicating with auto discovered device ?
	    const auto = this.accessories.find(x => x.context.address === address &&
					       x.context.eoj.toString() === eoj.toString() &&
					       x.context.reachable !== undefined)
	    if (auto) {
	      this.log(`Deleteing non-available accessory ${auto.displayName}`)
              this.api.unregisterPlatformAccessories(packageJson.name, "ELPlatform", [auto])
	      auto.context.reachable = false	// Prepare to remove
	    }
  ...
  async _addAccesory(address, eoj, uuid) {
    ...
    if (!registered) {
      ...
      const service = Object.keys(ELtype)
