...
class ELPlatform {
  ...
  async _init() {
	...
	const model = el.getClassName(eoj[0], eoj[1])
        const uuid = hap.uuid.generate(address + '|' + JSON.stringify(eoj))
	...
	if (existingAccessory) {
	  await buildAccessory(this, existingAccessory, ELcontroller[address], address, eoj)
	} else {
	  try {
	    const displayName = device.name ?? `${address}[${eoj[0]}:${eoj[1]}:${eoj[2]}].${getIpAddress()}`
	    ...
	    if (!await buildAccessory(this, accessory, ELcontroller[address], address, eoj)) {
	      this.log.error(`Unsupported device ${model} of ${address}[${eoj}].`)
	      continue  // unsupported accessory
	    }
	...
	if (!this.config.device.find(device =>
	  device.address === accessory.context.address &&
	    device.eoj.toString() === accessory.context.eoj.toString())) {
    ...
    el.on('notify', async (x) => {
      ...
      await this.onNotifyHandler[`${x.device.address}${x.message.seoj.toString()}`]?.(x.message)
    })
  }
