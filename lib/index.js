...
const ELtype = {
  heatercooler0:{default: true, eoj: [1, 0x30], builder: require('./accessory')},
  heatercooler:	{default: false, eoj: [1, 0x30], builder: require('./accessory-heatercooler')},
  thermostat:	{default: false, eoj: [1, 0x30], builder: require('./accessory-thermostat')},
  thermostat2:	{default: false, eoj: [1, 0x30], builder: async (platform, accessory, el, address, eoj) => await require('./accessory-thermostat')(platform, accessory, el, address, eoj, false)},
  lightbulb1:	{default: true, eoj: [2, 0x90], builder: require('./accessory')},
  lightbulb2:	{default: true, eoj: [2, 0x91], builder: require('./accessory')},
}
const HKdefaultService = (eoj) => {
  return Object.keys(ELtype)
    .filter(x => ELtype[x].default === true)
    .filter(x => ELtype[x].eoj[0] === eoj[0] && ELtype[x].eoj[1] === eoj[1])[0] ?? 'original'
}
...
class ELPlatform {
  ...
  async _init() {
	...
	if (typeof(device.instance) === 'number' && device.service) {
	  const eoj = ELtype[device.service.toLowerCase()]?.['eoj']
	  device.eoj = eoj ? [...eoj] : [,]
	  device.eoj.push(device.instance)
	}
	const address = device.address
	const eoj = device.eoj
	const service = device?.service?.toLowerCase() ?? HKdefaultService(eoj)
	...
	if (existingAccessory) {
	  await ELtype[service]?.['builder'](this, existingAccessory, ELcontroller[address], address, eoj)
	  if (existingAccessory.context.reachable !== undefined) {
	    this.log.debug(`${existingAccessory.displayName}: Discovered device transferred to manually configured device.`)
	    delete(existingAccessory.context.reachable)	// transfer auto discovered to manually configured device
	    this.log.debug(`${JSON.stringify(existingAccessory.context)}`)
	  }
	} else {
	  try {
	    ...
	    if (!await ELtype[service]?.['builder'](this, accessory, ELcontroller[address], address, eoj)) {
	...
	if (!this.config.device.find(device => {
	  ...
	  this.log(`Deleteing non-available accessory ${accessory.displayName}`)
          this.api.unregisterPlatformAccessories(packageJson.name, "ELPlatform", [accessory])
	  accessory.context.reachable = false	// Prepare to remove
	}
      }
      ...
      // }
      for (const accessory of this.accessories) {
	if (accessory.context.reachable === true) {	// except manually configured devices
	  accessory.context.reachable = false
	  await this._addAccesory(accessory.context.address, accessory.context.eoj, accessory.UUID)
	}
	if (accessory.context.reachable === false) {	// _addAccesory rejected
	  this.api.unregisterPlatformAccessories(packageJson.name, "ELPlatform", [accessory])
	}
      }
      this.accessories = this.accessories.filter(x => x.context.reachable !== false)
  ...
  async _startDiscovery() {
    ...
    return new Promise((resolve, reject) => {
	  ...
	  const service = HKdefaultService(eoj)
          const uid = service + '|' + address + '|' + JSON.stringify(eoj)
  ...
  async _stopDiscovery() {
    ...
    try {
      if (discoveredDevice.length > 0) {
	// load in the current config
	const {readFileSync, writeFileSync} = require('fs')
	const homebridgeConfig = JSON.parse(readFileSync(this.api.user.configPath(), 'utf8'))
	if (!Array.isArray(homebridgeConfig.platforms)) {
	  throw new Error('Cannot find platforms array in config for Homebridge')
	}
	const pluginConfig = homebridgeConfig.platforms.find(x => x.platform === "ELPlatform")
	if (!pluginConfig) {
	  throw new Error(`Cannot find config for ELPlatform in platforms array`)
	}
	pluginConfig.device ??= []
	pluginConfig.device = pluginConfig.device.concat(discoveredDevice)
	writeFileSync(this.api.user.configPath(), JSON.stringify(homebridgeConfig, null, 4))
	this.log.warn(`${this.api.user.configPath()} has been updated with adding discovered devices.`)
      }
    } catch (e) {
      this.log.error(`Failed to update config for ELPlatform. ${e}`)
    }
  ...
  async _addAccesory(address, eoj, uuid) {
    ...
    const service = HKdefaultService(eoj)
    const displayName = `${service}.${address}[${eoj[0]}:${eoj[1]}:${eoj[2]}].${getIpAddress()}`
    ...
    if (!registered) {
      ...
      const service = HKdefaultService(eoj)
      discoveredDevice.push({// name: accessory.displayName,
			     address: address,
			     instance: eoj[2],
			     service: service
			    })
