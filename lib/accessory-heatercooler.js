module.exports = async (platform, accessory, el, address, eoj) => {
  ...
  const mqttpublish = async (topic, message) => {
	...
	log.debug(`${accessory.displayName}: topic:${address}/${eoj[0]}:${eoj[1]}:${eoj[2]}/${topic}, message:${message}`)
      } catch (e) {
  ...
  .on('change', async (event) => {
	...
	const mode = context.TargetHeaterCoolerState
	await mqttpublish('mode', TargetHeaterCoolerState[mode])
	if (mode === hap.Characteristic.TargetHeaterCoolerState.HEAT) {
	  await mqttpublish('HeatingThresholdTemperature', context.HeatingThresholdTemperature)
	} else if (mode === hap.Characteristic.TargetHeaterCoolerState.COOL) {
	  await mqttpublish('CoolingThresholdTemperature', context.CoolingThresholdTemperature)
	}
      }
    }
  })
