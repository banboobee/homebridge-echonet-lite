module.exports = async (platform, accessory, el, address, eoj) => {
  ...
  const propertyMaps = await new Promise(async (resolve, reject) => {
    const timeout = setTimeout(() => {
      reject(new Error(`timed out to connect echonet-lite protocol.`))
    }, 3*1000)
    EL(el.getPropertyMaps, address, eoj)
      .then((x) => {
  ...
  let tid = -1
  platform.onNotifyHandler[`${context.address}${context.eoj.toString()}`] = async (x) => {
    try {
      if (x['esv'] === 'INF') {
	if (tid < parseInt(x['tid'])) {
	  tid = parseInt(x['tid'])
	  const dummy = async (x, y) => {}
	  ...
	  case 0xB0:	//Target state 
	    ...
	    service.updateCharacteristic(hap.Characteristic.TargetHeaterCoolerState, target)
	    updateCurrentHeaterCoolerState(dummy)
	    break
  ...
  const updateCurrentHeaterCoolerState = async (callback) => {
    try {
      if (!context.Active) {
  ...
  const syncTargetHeaterCoolerState = async (callback) => {
    try {
  ...
  const syncCoolingThresholdTemperature = async (callback) => {
    try {
      const edt = propertyMaps.get.find(x => x === 0xB5) ? 0xB5 : 0xB3
  ...
  .on('set', async (value, callback) => {
    try {
      ...
      log.error(`Failed to set HeatingThresholdTemperature of ${accessory.displayName}. ${err}`)
      callback(err)
  ...
  const syncHeatingThresholdTemperature = async (callback) => {
    try {
      const edt = propertyMaps.get.find(x => x === 0xB6) ? 0xB6 : 0xB3
