...
module.exports = async function(platform, accessory, el, address, eoj) {
  ...
  this.state = new Proxy(state, {
	...
	if (key == 'targetCoolingThresholdTemperature') {
	  await this.mqttpublish('CoolingThresholdTemperature', value)
	} else if (key == 'targetHeatingThresholdTemperature') {
	  await this.mqttpublish('HeatingThresholdTemperature', value)
	} else if (key == 'active') {
	  if (!value) {
	    await this.mqttpublish('mode', 'off')
	  } else {
	    const {mode} = (await EL(el.getPropertyValue, this.address, this.eoj, 0xB0)).message.data
	    const {target, current} = mode2hap[mode]
	    await this.mqttpublish('mode', mqttmode[target])
	  }
	} else if (key == 'targetHeaterCoolerState') {
	  const mode = mqttmode[value]
	  await this.mqttpublish('mode', mode)
	  if (mode === 'heat') {
	    await this.mqttpublish('HeatingThresholdTemperature', this.state.targetHeatingThresholdTemperature)
	  } else if (mode === 'cool') {
	    await this.mqttpublish('CoolingThresholdTemperature', this.state.targetCoolingThresholdTemperature)
	  }
  ...
  // const mqttmode = ['off', 'auto', 'heat', 'cool']
  const mqttmode = ['auto', 'heat', 'cool']
  ...
  const mode2hap = [
    ...
    // {current:undefined,							target:hap.Characteristic.TargetHeaterCoolerState.AUTO},
